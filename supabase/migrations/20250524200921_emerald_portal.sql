/*
  # Churn Prediction System Schema

  1. New Tables
    - `churn_predictions` - Stores predictions about user churn probability
    - `churn_model_metrics` - Stores metrics about the ML model performance
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_attribute WHERE attrelid = 'public.users'::regclass AND attname = 'role' AND NOT attisdropped) THEN
    ALTER TABLE public.users ADD COLUMN role text NOT NULL DEFAULT 'member' CHECK (role IN ('member', 'staff', 'admin'));
  END IF;
END$$;
